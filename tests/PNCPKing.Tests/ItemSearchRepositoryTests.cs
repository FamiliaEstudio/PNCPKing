...
public sealed class ItemSearchRepositoryTests
{
    ...
    public async Task NearbyFilter_UsesCatalogCodesAndOrdersByStraightLineDistance()
    {
        ...
        Assert.Equal("ribeirao", result.Results[0].PncpId);
        Assert.Equal(0d, result.Results[0].DistanceFromRibeiraoKilometers);
        Assert.Equal("itamogi", result.Results[1].PncpId);
        Assert.True(result.Results[1].DistanceFromRibeiraoKilometers > 0d);
        Assert.DoesNotContain(result.Results, contract => contract.PncpId == "name-only");
    }
