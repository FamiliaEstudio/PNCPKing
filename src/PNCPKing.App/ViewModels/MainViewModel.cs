...
public sealed partial class MainViewModel : ObservableObject, IAsyncDisposable
{
    ...
    private readonly NationalPriceIndexService _nationalPriceIndexService;
    ...
    private CancellationTokenSource? _aggressivePriceCacheIterationCancellation;
    ...
    private bool _isLocalPricePageLoading;
    private bool _localPricesNeedRefresh;
    private int _localPriceDataVersion;
    private bool _contractResultsLoaded;
    private int _contractResultsGeneration;
    private int _loadingContractResultsGeneration = -1;
    private bool _hasMoreLocalPriceRows;
    private int _localPriceRowsLoaded;
    private ItemSearchResultCursor? _temporaryPriceCursor;
    private bool _temporaryPricePagingStarted;
    private bool _hasMoreTemporaryPriceRows;
    private int _temporaryPriceRowsLoaded;
    private bool _remotePriceExpansionStarted;
    private bool _isStalePriceRevalidationActive;
    private bool _restartPriceSessionOnNextExpansion;
    ...
    public MainViewModel(
        ...
        NationalPriceIndexService nationalPriceIndexService,
        ...
        SqliteCalibrationService calibrationService)
    {
        ...
        _nationalPriceIndexService = nationalPriceIndexService;
        ...
        _selectedResourceProfile = calibrationService.Connections.SelectedResourceProfile;
        ...
        SearchCommand = new AsyncRelayCommand(
            ...
            () => !IsAnyAggressivePncpMode && !IsFileBusy && !_isResultPageLoading,
            allowConcurrentExecutions: true);
        RestartItemSearchCommand = new AsyncRelayCommand(
            RestartItemSearchAsync,
            () => !IsAnyAggressivePncpMode && !IsFileBusy && !IsPriceBusy && !_isResultPageLoading &&
        ...
        ApplyPriceFilterCommand = new AsyncRelayCommand(
            ...
            () => !IsFileBusy && !IsPriceBusy && !_isResultPageLoading && !_isLocalPricePageLoading &&
                  _activeItemSearchExpression is not null);
        StopItemSearchCommand = new RelayCommand(StopItemSearch, () => _isItemSearchActive);
        CalculatePreflightCommand = new AsyncRelayCommand(
            CalculatePreflightAsync,
            () => !IsAnyAggressivePncpMode && !IsFileBusy && !IsIndexBusy);
        ...
        HydrateCommand = new AsyncRelayCommand(
            ...
            () => !IsAnyAggressivePncpMode && !IsFileBusy && !IsForegroundBusy && SelectedContract is not null);
        ...
        AccessDocumentsCommand = new AsyncRelayCommand<ContractRecord>(
            ...
            contract => !IsAnyAggressivePncpMode && contract is not null && !IsDocumentBusy && !IsFileBusy);
        AccessItemDocumentsCommand = new AsyncRelayCommand<ItemSearchDisplayRow>(
            AccessItemDocumentsAsync,
            row => !IsAnyAggressivePncpMode && row is not null && !IsDocumentBusy && !IsFileBusy);
        ...
        SelectResourceProfileCommand = new AsyncRelayCommand<string>(SelectResourceProfileAsync);
    ...
    public SearchSortOption SelectedSortOption
    {
        ...
        set
        {
            ...
            _ = EnsureContractResultsAsync();
        }
    }

    public int ItemSearchGeneration => _contractSearchGeneration;
    ...
    public void ClearItemPriceRetention(ItemSearchDisplayRow? row)
    {
        if (row is null)
        {
            return;
        ...
        if (!row.IsPinned && !row.IsSelectedForBasket)
        {
            StatusText = "Preço já está desmarcado e sem fixação.";
            return;
        }
        ...
        SynchronizeRetainedItemRow(row);
        StatusText = "Preço desmarcado e fixação removida.";
    }
    ...
    private void SetIndexUpdateStage(int stage)
    {
        _indexUpdateStage = stage;
        OnPropertyChanged(nameof(IndexActivityText));
    ...
    public async Task ShutdownAsync()
    {
        ...
        _documentCancellation?.Cancel();
        _fileOperationCancellation?.Cancel();
        if (_gitHubUpdateTask is { } updateTask)
        {
            try { await updateTask.ConfigureAwait(true); }
            catch (Exception exception) when (!AsyncCommandRuntime.IsCritical(exception)) { }
        }
        ...
        _isAggressiveNationalPriceMode = false;
        _aggressiveNationalPriceIterationCancellation?.Cancel();
        ...
        if (_nationalPriceCycleTask is { } nationalPriceTask)
        {
            try
            {
                await nationalPriceTask.ConfigureAwait(true);
            }
            catch (OperationCanceledException)
            {
                // Normal application shutdown.
        ...
        bool revalidateStalePrices = false)
    {
        ...
        if (resetSession)
        {
            ...
            _isStalePriceRevalidationActive = false;
            SetPriceBusy(false, usesNetwork: false);
            _restartPriceSessionOnNextExpansion = restartPriceSession;
        }
        ...
        InvalidateContractResults();
        OnPropertyChanged(nameof(ItemSearchGeneration));
        try
        {
            ...
            SetItemSearchActive(false);
            await RemoveInvalidRetainedPricesAsync(activePriceCancellation.Token).ConfigureAwait(true);
            ResetCurrentItemRows();
            CurrentItemPage = 0;
            ...
            _temporaryPriceCursor = null;
            _temporaryPricePagingStarted = false;
            _hasMoreTemporaryPriceRows = false;
            _temporaryPriceRowsLoaded = 0;
        ...
        {
            if (searchGeneration == Volatile.Read(ref _contractSearchGeneration))
                StatusText = "Pesquisa interrompida; os preços encontrados foram preservados.";
        }
        catch (Exception exception)
        {
            if (searchGeneration == Volatile.Read(ref _contractSearchGeneration))
    ...
    private void InvalidateContractResults(bool clearRows = true)
    {
        _contractResultsGeneration++;
        _contractResultsLoaded = false;
        _contractPageHasMore = false;
        _contractCountCancellation?.Cancel();
        _contractSearchCountPending = false;
        _contractSearchCountExact = false;
        ...
        if (clearRows) ContractResults.Clear();
        OnPropertyChanged(nameof(ContractPageSummary));
        NotifyCommands();
    }

    private async Task EnsureContractResultsAsync()
    {
        if (!IsContractsPanelOpen || ContractPanelViewIndex != 0 || _activeSearchQuery is null ||
            _contractResultsLoaded || _loadingContractResultsGeneration == _contractResultsGeneration) return;
        var generation = _contractResultsGeneration;
        _loadingContractResultsGeneration = generation;
        try
        {
            await LoadContractPageAsync(1, cancellationToken: _contractSearchCancellation?.Token ?? _startupCancellation.Token)
                .ConfigureAwait(true);
        }
        catch (OperationCanceledException) { }
        catch (Exception exception)
        {
            if (generation == _contractResultsGeneration)
                StatusText = $"Não foi possível carregar as contratações: {exception.Message}";
        }
        finally
        {
            if (_loadingContractResultsGeneration == generation) _loadingContractResultsGeneration = -1;
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var resultsGeneration = _contractResultsGeneration;
        ...
        var result = await Task.Run(
                ...
                cancellationToken)
            .ConfigureAwait(true);
        if (generation != Volatile.Read(ref _contractSearchGeneration) || resultsGeneration != _contractResultsGeneration)
        {
            return;
        }

        using var applySpan = _performanceTelemetry.Begin("ui", "contract-results-apply");
        var selectedContractId = SelectedContract?.PncpId;
        ContractResults.ReplaceAll(result.Results);
        SelectedContract = result.Results.FirstOrDefault(contract => contract.PncpId == selectedContractId);
        _contractResultsLoaded = true;
    ...
    private async Task RevalidateStalePricesAsync(
        SearchQuery query,
        SearchExpression expression,
        int searchGeneration,
        CancellationToken cancellationToken)
    {
        using var span = _performanceTelemetry.Begin("price-search", "stale-revalidation");
        _isStalePriceRevalidationActive = true;
        SetPriceBusy(true, usesNetwork: true);
        try
        {
            var progress = new Progress<StalePriceRevalidationProgress>(value =>
            {
                if (searchGeneration != Volatile.Read(ref _contractSearchGeneration))
                {
                    return;
                ...
                PriceSearchProgress = value.Percentage;
                ItemSearchSummary = value.TotalContracts == 0
                    ? $"{_localPriceRowsLoaded:N0} preços locais atuais exibidos; nenhum preço alterado aguarda revalidação."
                    : $"Revalidando {value.TotalContracts:N0} contratações alteradas no PNCP… " +
                      $"{value.ProcessedContracts:N0}/{value.TotalContracts:N0} concluída(s); " +
                      $"chamadas de resultados: {value.ItemResultCalls:N0}; falhas: {value.FailedCalls:N0}.";
                StatusText = value.TotalContracts == 0
                    ? "Resultados locais atuais entregues."
                    : $"Revalidando preços alterados no PNCP: {value.ProcessedContracts:N0}/" +
                      $"{value.TotalContracts:N0} contratação(ões).";
            });
            var result = await _itemSearchService.RevalidateStaleMatchesAsync(
                    query,
                    expression,
                    progress,
                    cancellationToken)
            ...
            if (result.TotalContracts == 0)
            {
                ...
                ItemSearchSummary =
                    $"{_localPriceRowsLoaded:N0} preços locais atuais exibidos; " +
                    "nenhum preço alterado aguarda revalidação.";
                StatusText = "Resultados locais atuais entregues.";
                span.Complete();
                return;
            }

            await ReloadLocalPricesAtomicallyAsync(cancellationToken).ConfigureAwait(true);
            PriceSearchProgress = 100;
            ItemSearchSummary =
                $"Revalidação concluída; resultados locais atualizados. " +
                $"{result.ProcessedContracts:N0} contratação(ões) examinada(s), " +
                $"{result.ItemResultCalls:N0} resultado(s) consultado(s) e " +
                $"{result.FailedCalls:N0} falha(s).";
            StatusText = "Revalidação concluída; resultados locais atualizados.";
            span.Complete(result.ProcessedContracts);
        }
        catch (OperationCanceledException)
        {
            if (searchGeneration == Volatile.Read(ref _contractSearchGeneration))
            {
                ItemSearchSummary =
                    $"Revalidação interrompida; {_localPriceRowsLoaded:N0} preços locais atuais permanecem exibidos.";
                StatusText = "Revalidação de preços interrompida; checkpoints preservados.";
            }
            span.Complete();
        }
        catch (Exception exception)
        {
            ...
            {
                ItemSearchSummary =
                    $"Os preços locais atuais permanecem exibidos. Falha ao revalidar alterações: {exception.Message}";
                StatusText = "Revalidação adiada; será tentada novamente em uma nova pesquisa.";
            }
            span.Fail(exception);
        }
        finally
    ...
    {
        if (_disposed || _isResultPageLoading || _isLocalPricePageLoading)
        {
            return;
        }

        if (!HasMoreStoredPriceRows)
        {
            return;
        }

        _isResultPageLoading = true;
        if (_priceCancellation is null || _priceCancellation.IsCancellationRequested)
        {
        ...
        try
        {
            ItemSearchSummary = "Carregando mais preços salvos; pesquisa local em andamento…";
            var cancellationToken = _priceCancellation?.Token ?? _startupCancellation.Token;
            if (_localPricesNeedRefresh)
            {
                await RemoveInvalidRetainedPricesAsync(cancellationToken).ConfigureAwait(true);
                await ReloadLocalPricesAtomicallyAsync(cancellationToken).ConfigureAwait(true);
                if (_localPricesNeedRefresh) return;
            }
        ...
        {
            ItemSearchSummary =
                "Carregamento interrompido; as linhas e o cursor do último bloco concluído foram preservados.";
            StatusText = ItemSearchSummary;
        }
        finally
        {
            _isResultPageLoading = false;
            NotifyCommands();
        }
    }
    ...
    private async Task FireBatchesAsync()
    {
        ...
        if (query is null || expression is null)
        {
            ItemSearchSummary = "Pesquise um item antes de ampliar pela API.";
            return;
        }
        ...
        {
            ItemSearchSummary = "Calculando o total de contratações candidatas para esgotar pela API…";
            await RefreshLocalItemSearchSummaryAsync(
                    query,
                    expression,
                    Volatile.Read(ref _contractSearchGeneration),
                    cancellation.Token)
            ...
            {
                ItemSearchSummary =
                    $"{_localPriceRowsLoaded:N0} preços locais atuais preservados; " +
                    "a ampliação pela API não foi iniciada.";
                return;
            }
        }
        ...
        if (!_remotePriceExpansionStarted)
        {
            ItemSearchSummary = "Preparando a sessão explícita de ampliação pela API…";
            await _itemSearchService.StartAsync(
                    query with { Page = 1, PageSize = 200 },
                    _restartPriceSessionOnNextExpansion,
                    cancellation.Token)
                .ConfigureAwait(true);
            _restartPriceSessionOnNextExpansion = false;
        ...
        bool exhaustCandidateSet = false)
    {
        if (IsAnyAggressivePncpMode)
        {
            ItemSearchSummary = "Desative o download agressivo antes de consultar preços no PNCP.";
            return;
        }

        using var expansionSpan = _performanceTelemetry.Begin("price-search", "explicit-api-expansion");
        ...
        try
        {
            ...
            var rowProgress = new Progress<IReadOnlyList<ItemSearchRow>>(rows =>
            {
                ...
                streamedRows += AppendUniqueRows(rows);
                ItemSearchSummary =
                    $"{streamedRows:N0} preço(s) novo(s) recebido(s) durante os lotes da API; continuando…";
            });
            ...
            if (runGeneration == Volatile.Read(ref _priceRunGeneration))
            {
                ...
                _hasMoreTemporaryPriceRows =
                    result.AvailableSessionRows > _temporaryPriceRowsLoaded;
                NotifyCommands();
            }
            ItemSearchSummary =
            ...
            expansionSpan.Complete(result.ProcessedContracts);
        }
        catch (OperationCanceledException)
        {
            if (runGeneration == Volatile.Read(ref _priceRunGeneration))
    ...
    private async Task ApplyPriceFilterAsync()
    {
        ...
        try
        {
            ...
            if (generation != Volatile.Read(ref _contractSearchGeneration)) return;
            ItemSearchSummary =
    ...
    private async Task ReloadAllDiscoveredRowsAsync(CancellationToken cancellationToken)
    {
        ...
        var (minimum, maximum) = ParsePriceRange();
        var pagesToRestore = Math.Max(1, CurrentItemPage);
        _localPricePage = 0;
        _localPriceCursor = null;
        _localPricesNeedRefresh = false;
        _hasMoreLocalPriceRows = false;
        ...
        CurrentItemPage = 0;
        ResetCurrentItemRows();
        for (var index = 0; index < pagesToRestore; index++)
        {
            await LoadNextStoredPricePageAsync(cancellationToken, minimum, maximum)
                .ConfigureAwait(true);
            if (generation != Volatile.Read(ref _contractSearchGeneration) || dataVersion != _localPriceDataVersion) return;
            if (!HasMoreStoredPriceRows)
            {
                break;
            }
    ...
    private void InvalidateLocalPriceCursor()
    {
        _localPriceDataVersion++;
        _itemResultBuffer.Clear();
        _localPendingApplyKeys.Clear();
        _localPriceCursor = null;
        _localPricePage = 0;
        _localPricesNeedRefresh = _activeItemSearchExpression is not null;
        _hasMoreLocalPriceRows = _localPricesNeedRefresh;
        InvalidateContractResults(clearRows: false);
        if (!IsFileBusy && !IsIndexBusy && !IsForegroundBusy) _ = EnsureContractResultsAsync();
    }
    ...
    private async Task ReloadLocalPricesAtomicallyAsync(CancellationToken cancellationToken)
    {
        ...
        var expression = _activeItemSearchExpression;
        var pagesToRestore = Math.Max(1, CurrentItemPage);
        var rows = new List<ItemSearchRow>(pagesToRestore * ItemSearchSessionService.DefaultPageSize);
        PriceCacheLocalCursor? cursor = null;
        PriceCacheLocalPage? lastPage = null;
        for (var pageNumber = 0; pageNumber < pagesToRestore; pageNumber++)
        {
            lastPage = await Task.Run(
        ...
        _localPricesNeedRefresh = false;
        _localPricePage = lastPage?.Page ?? 0;
        _localPriceCursor = lastPage?.Cursor;
        _hasMoreLocalPriceRows = lastPage?.HasMore == true;
        _localPriceRowsLoaded = rows.Count;
        CurrentItemPage = rows.Count > 0 ? Math.Max(1, _localPricePage) : 0;
        HasMoreItemCandidates = HasMoreStoredPriceRows;
        NotifyCommands();
    }

    private void ReplaceVisiblePriceRows(IReadOnlyList<ItemSearchRow> rows)
    {
        using var span = _performanceTelemetry.Begin("ui", "item-results-atomic-apply");
        var selectedKey = SelectedItemSearchRow is { } selected ? RowKey(selected) : null;
        _itemResultBuffer.Clear();
        var replacement = new List<ItemSearchDisplayRow>(_retainedItemRows.Count + rows.Count);
        _visibleItemKeys.Clear();
        _currentItemResultKeys.Clear();
        foreach (var (key, retained) in _retainedItemRows)
        {
                     ...
                     _activeItemSearchExpression.MatchesItem(row.Item.Description, row.Item.Unit)))
        {
            ...
            if (_visibleItemKeys.Add(key))
            {
                replacement.Add(_retainedItemRows.TryGetValue(key, out var retained) ? retained : display);
            }
        }

        ItemSearchRows.ReplaceAll(replacement);
        SelectedItemSearchRow = replacement.FirstOrDefault(row => RowKey(row) == selectedKey);
        span.Complete(replacement.Count);
    }

    private async Task<int> LoadNextStoredPricePageAsync(
        CancellationToken cancellationToken,
        ...
        decimal? maximum = null)
    {
        ...
        var remaining = ItemSearchSessionService.DefaultPageSize;
        var loaded = 0;
        while (remaining > 0 && (_localPricePage == 0 || _hasMoreLocalPriceRows))
        {
            var previousCursor = _localPriceCursor;
            var local = await LoadLocalPricePageAsync(
                    cancellationToken,
                    minimum,
                    maximum,
                    remaining,
                ...
                .ConfigureAwait(true);
            loaded += local;
            remaining = Math.Max(0, remaining - local);
            if (!_hasMoreLocalPriceRows || Equals(previousCursor, _localPriceCursor))
            {
                break;
            }
        }

        if (remaining > 0 && _remotePriceExpansionStarted &&
            (!_temporaryPricePagingStarted || _hasMoreTemporaryPriceRows))
        {
            loaded += await LoadTemporaryPricePageAsync(
                    remaining,
                    cancellationToken,
                    minimum,
                    maximum,
        ...
        HasMoreItemCandidates = HasMoreStoredPriceRows || !_remotePriceExpansionStarted;
        NotifyCommands();
        return loaded;
    }
    ...
    private async Task<int> LoadLocalPricePageAsync(
        ...
        decimal? minimum = null,
        decimal? maximum = null,
        int pageSize = ItemSearchSessionService.DefaultPageSize,
        ...
        int? actionTargetRows = null)
    {
        ...
        var requestedRows = Math.Clamp(pageSize, 1, ItemSearchSessionService.DefaultPageSize);
        var targetRows = actionTargetRows ?? requestedRows;
        var generation = Volatile.Read(ref _contractSearchGeneration);
        var dataVersion = _localPriceDataVersion;
        var loaded = 0;
        ...
        var progress = new Progress<PriceCacheLocalProgress>(value =>
        {
            ...
            _localPriceCursor = value.Cursor;
            _localPricePage = value.Cursor?.Page ?? _localPricePage;
            _hasMoreLocalPriceRows = value.HasMore;
            if (!firstProgressRecorded)
            {
                firstProgressRecorded = true;
                _performanceTelemetry.Record(
                    "price-search",
                    "local-first-ui-progress",
            ...
            {
                firstRowRecorded = true;
                _performanceTelemetry.Record(
                    "price-search",
                    "local-first-visible-row",
                    Stopwatch.GetElapsedTime(loadStarted),
                    added);
            }
            var message =
            ...
            ItemSearchSummary = value.Completed ? message : message +
                (loaded >= requestedRows ? "; confirmando se há mais resultados…" : "; continuando…");
            StatusText = ItemSearchSummary;
        ...
        try
        {
            ...
            await _itemResultBuffer.FlushAsync().ConfigureAwait(true);
            if (generation != Volatile.Read(ref _contractSearchGeneration) || dataVersion != _localPriceDataVersion) return 0;
    ...
    private async Task<int> LoadTemporaryPricePageAsync(
        int pageSize,
        CancellationToken cancellationToken,
        decimal? minimum = null,
        decimal? maximum = null,
        ...
        int? actionTargetRows = null)
    {
        if (pageSize <= 0 || _itemSearchService.CurrentSession is null)
        {
            return 0;
        }
        ...
        var loaded = 0;
        var remaining = Math.Clamp(pageSize, 1, ItemSearchSessionService.DefaultPageSize);
        while (remaining > 0 && (!_temporaryPricePagingStarted || _hasMoreTemporaryPriceRows))
        {
            var page = await _itemSearchService.LoadDiscoveredResultPageAsync(
                    _temporaryPriceCursor,
                    remaining,
                    minimum,
                    maximum,
                    cancellationToken)
                .ConfigureAwait(true);
            var added = AppendUniqueRows(page.Rows);
            loaded += added;
            remaining -= added;
            _temporaryPricePagingStarted = true;
            _temporaryPriceCursor = page.NextCursor;
            _hasMoreTemporaryPriceRows = page.HasMore;
            _temporaryPriceRowsLoaded += page.Rows.Count;
            var totalActionRows = actionRowsAlreadyLoaded + loaded;
            var targetRows = actionTargetRows ?? pageSize;
            ItemSearchSummary =
                $"{totalActionRows:N0}/{targetRows:N0} preços encontrados nos resultados salvos" +
                (page.HasMore && remaining > 0 ? "; continuando…" : string.Empty);
            StatusText = ItemSearchSummary;
    ...
    private void StopItemSearch()
    {
        ...
        var wasRevalidating = _isStalePriceRevalidationActive;
        Interlocked.Increment(ref _priceRunGeneration);
        _activeExpansionRemaining = 0;
        _activeExpansionExhaustive = false;
        _priceCancellation?.Cancel();
        _itemSearchService.Stop();
        _hasMoreTemporaryPriceRows = _itemSearchService.CurrentSession is not null;
        SetPriceBusy(false, usesNetwork: false);
        SetItemSearchActive(false);
        HasMoreItemCandidates = false;
        if (wasActive)
        {
            ItemSearchSummary = wasRevalidating
    ...
    private int AppendUniqueRows(IEnumerable<ItemSearchRow> rows, bool localPage = false)
    {
        ...
        _itemResultBuffer.Enqueue(pending);
    ...
    private async Task RemoveInvalidRetainedPricesAsync(CancellationToken cancellationToken)
    {
        foreach (var (key, retained) in _retainedItemRows.ToArray())
        {
            var stored = await _repository.GetCachedItemResultsAsync(
                    retained.Contract.PncpId,
                    retained.Item.ItemNumber,
                    cancellationToken)
                .ConfigureAwait(true);
            var retainedResult = retained.Result;
            var currentResult = retainedResult is null
                ? null
                : stored?.Results.FirstOrDefault(result =>
                    result.ResultSequence == retainedResult.ResultSequence);
            var remainsCurrent = stored?.IsCurrent == true && currentResult is not null &&
                                 currentResult.ResultStatusId == retainedResult?.ResultStatusId &&
                                 currentResult.HomologatedQuantityScaled == retainedResult?.HomologatedQuantityScaled &&
                                 currentResult.HomologatedUnitValueScaled == retainedResult?.HomologatedUnitValueScaled &&
                                 currentResult.HomologatedTotalValueScaled == retainedResult?.HomologatedTotalValueScaled;
            if (remainsCurrent)
            {
                continue;
            }

            retained.IsPinned = false;
            retained.IsSelectedForBasket = false;
            _retainedItemRows.Remove(key);
        }
    ...
    private void UpdateItemSearchProgress(PriceBatchProgress progress, int? runGeneration = null)
    {
        ...
        _hasMoreTemporaryPriceRows =
            progress.AvailableSessionRows > _temporaryPriceRowsLoaded;
        ...
        ItemSearchSummary =
            ...
            $"parcela local: {progress.CachedContractsProcessedInSlice:N0}/" +
            $"{Math.Max(1, progress.CachedContractsPerSlice):N0}; " +
            $"parcelas concluídas: {progress.CompletedCacheSlices:N0}; " +
            ...
            $"resultados temporários disponíveis: {progress.AvailableSessionRows:N0}; " +
    ...
    private string BuildLocalSearchSummary() => _localItemSearchSummary is null
        ...
        : $"Banco local: {_localItemSearchSummary.CandidateContracts:N0} contratação(ões) candidata(s) (total exato); " +
          ...
          $"{_localPriceRowsLoaded:N0} preço(s) permanente(s) e " +
          $"{_temporaryPriceRowsLoaded:N0} temporário(s) exibido(s).";
        ...
        CancellationToken visibleActivityCancellation)
    {
        ...
        try
        {
            await RunIndexOperationAsync(async cancellationToken =>
            {
                ...
                {
                    sliceCancellation.CancelAfter(budget);
                }
                try
                {
    ...
    private IProgress<SyncProgress> CreateSyncProgress(string prefix = "") => new Progress<SyncProgress>(item =>
    {
        OperationProgress = item.Percentage;
        StatusText = $"{prefix}{item.Message} - {item.ContractsSaved:N0} registro(s) gravado(s)";
        ...
        bool supportsPause = false)
    {
        ...
        catch (Exception exception)
        {
            ...
            if (HasHttpStatus(exception, HttpStatusCode.GatewayTimeout) && !_disposed)
            {
        ...
        finally
        {
            ...
            IsIndexBusy = false;
            SetIndexUpdateStage(0);
            if (supportsPause) InvalidateLocalPriceCursor();
            NotifyCommands();
        }
    }
        ...
        foreach (var command in new ICommand[]
                 {
                     ...
                     RemovePriceCacheCommand, ToggleAggressivePriceCacheCommand,
                     EstimateAndActivateNationalPriceIndexCommand,
                     PauseNationalPriceIndexCommand, CancelNationalPriceIndexCommand,
                     DisableNationalPriceIndexCommand, RemoveNationalPriceIndexCommand,
                     ...
                     ToggleQuotationMedicationCommand, OrganizeQuotationItemsCommand, ManageQuotationGroupsCommand,
    ...
    private static bool HasHttpStatus(Exception exception, HttpStatusCode statusCode) =>
        exception is HttpRequestException requestException && requestException.StatusCode == statusCode ||
        exception.InnerException is not null && HasHttpStatus(exception.InnerException, statusCode);

    private static string GetSynchronizationErrorMessage(Exception exception)
    {
        if (HasHttpStatus(exception, HttpStatusCode.TooManyRequests))
        {
            return "O PNCP interrompeu as consultas por excesso de requisições (erro 429). " +
                   "O progresso foi preservado. Aguarde alguns minutos e use Atualizar para retomar " +
                   "a sequência de contratações, itens e preços.";
        }

        if (!HasHttpStatus(exception, HttpStatusCode.GatewayTimeout))
        {
